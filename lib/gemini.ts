...
//   → **優先順位のリスト（PREFERRED）の中だけを順に試し、すべて使えなければそこで止める**
//   （2026-09-10 P7-d で、一覧の他のモデルへ流さないよう範囲を限定した。下の `preferredOnly`）。
...
/**
 * 優先して使うモデル（この順に試す）。一覧に無いものは候補から外し、呼んで失敗したら次へ進み、
 * すべて駄目なら**そこで止める**（一覧の他のモデルへは流さない。`preferredOnly`）。
 *
