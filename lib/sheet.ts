...
// [DECISION 2026-09-17] 押印欄を用紙の右上に置いた（P9・6-b）。
//   → [DECISION 2026-09-23] **用紙から外し、完成形（Word・PDF）の1ページ目の右上へ移した**（P12・設計側）。押印は完成した記録の
//     承認に押すもので、白紙の用紙を書く段階では要らない。ラベルと寸法の定義は `lib/stamp.ts` だけにある（ここには残さない）。
//     頭は1列になり、題・日時・場所の行が右端まで使う（場所 約182mm・会議名の下線も右端まで）。
//     頭の高さは変わらない（押印欄は記入欄より低く、高さを決めていなかった）ので、罫線の本数も変わらない。
// [DECISION 2026-09-17] **記入欄は1項目1行**（日時／場所／参加者・出席者。P9-b・設計側の指示）。
//   押印欄が入って「場所」の下線が面談 約64mm・会議 約49mm まで狭くなり、施設名の長い場所や出席者の多い会議で足りなかった
//   （P12 で押印欄を外したあとも1項目1行のまま）。
...
/** 記入欄の頭の文言（種類ごと）。題・参加者の欄の見出し（押印欄は P12 で完成形へ移した＝ `lib/stamp.ts`） */
export const SHEET_HEAD: Record<RecordType, { title: string; people: string; name?: string }> = {
  interview: { title: "面談記録メモ", people: "参加者" },
  /** name: 題の右に置く欄の見出し（会議だけ。P9-c） */
  meeting: { title: "会議記録メモ", people: "出席者", name: "会議名" },
};
