// 人名検知の保険（原則3: 赤は伏せ忘れの知らせ）。
//
// AI が人名を "p"/"y"/"b" に紛れ込ませたときのために、敬称付きの氏名パターンを機械的に検出して
// 独立した "r" トークンに切り出す。AI 側の検知を置き換えるものではなく、取りこぼしを拾う二重防御。
// [DECISION 2026-09-23・設計側] 赤は「送った画像に残った（塗り忘れの）人名」を知らせる印（P15）。置き換えは求めず、
//   確認画面で1件ずつ確認するまで完成できない。**面談・会議とも**通す（route.ts）。
//   塗りつぶした箇所の「〇〇」（〇 U+3007・○ U+25CB・◯ U+25EF）は漢字の範囲（一-龯）に入らないので拾わない
//   （`tests/names.test.mts`）。
//
