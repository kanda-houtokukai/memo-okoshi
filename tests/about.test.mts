...
const GOLDEN = {
  ...
  leads: {
    ...
    mask: "氏名などを隠します。隠したあとの画像だけが送られます。会議では出席者や発言者の名前を記録に残します。利用者の氏名など、送りたくない語を選んで隠してください。",
  ...
  meeting: {
    ...
    /** 手順3（1枚に入る項目の数・記入欄と押印欄の付く枚） */
    prepStep3: "1枚に入る項目は6つまでです。7つ以上になると次の用紙に続きます。記入欄と押印欄は1枚目にだけ付きます。",
    /** 赤の説明と「完成」（**面談だけ**の話であることを明示する） */
...
test("会議への対応が文面に入っている（章は足さず、該当する章に一文ずつ・P10）", () => {
  ...
  // 面談だけの話には「会議では出ない」を添える（原則3は面談のみ）
  ...
  assert.ok(ABOUT.cautions.find((c) => c.b === "伏せ忘れの確認")?.s.includes("会議では印が出ません"));
